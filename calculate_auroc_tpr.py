...
def main():
    ...
    # AUROC
    human_z = [r['z_score'] for r in human_results[args.task]['raw_detection_results']]
    machine_z = [r['z_score'] for r in machine_results[args.task]['raw_detection_results']]
    ...
    task_results = deepcopy(machine_results[args.task])
    raw_detection_results = task_results.pop('raw_detection_results')
    task_results['roc_auc'] = roc_auc
    task_results['TPR (FPR = 0%)'] = tpr_value0
    task_results['TPR (FPR < 1%)'] = tpr_value1
    task_results['TPR (FPR < 5%)'] = tpr_value5
    task_results['raw_detection_results'] = raw_detection_results
    task_results['neg_samples_file'] = args.human_fname
    machine_results[args.task] = task_results
